...
from common.mujoco_envs_gym import MujocoEnv
from reward_function import world_rfc_implicit_reward_gym
...
class HumanoidGymTemplate(MujocoEnv, EzPickle):
    metadata = {
        "render_modes": [
            "human",
            "rgb_array",
    ...
    def __init__(self, cfg, 
        ...
        EzPickle.__init__(self,cfg,frame_skip,
                           default_camera_config,
                          **kwargs)
        self.cfg = cfg
        self.end_reward = 0.0
    ...
    def set_spaces(self):
        ...
        self.obs_dim = self.get_obs()['agent'].size
        high = np.inf * np.ones(self.obs_dim)
        low = -high
        obs_space = gym.spaces.Dict({
            'agent': gym.spaces.Box(low, high, dtype=np.float32)
        })
    ...
    def step(self, a):
        ...
        reward,reward_info = world_rfc_implicit_reward_gym(self,a)
